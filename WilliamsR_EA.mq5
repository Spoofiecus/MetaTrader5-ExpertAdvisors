...
input int                MagicNumber = 303030;   // Magic Number
...
//--- get Williams %R handle
   wpr_handle = iWPR(_Symbol, _Period, WPRPeriod);

   if(wpr_handle == INVALID_HANDLE)
     {
      printf("Error creating Williams %%R indicator");
      return(INIT_FAILED);
     }
...
//--- get WPR values for the last 2 completed bars
   if(CopyBuffer(wpr_handle, 0, 1, 2, wpr_buffer) != 2)
     {
      printf("Error copying Williams %%R buffer");
      return;
     }
...
//--- check for buy signal (WPR returns from oversold)
   if(wpr_buffer[1] <= OversoldLevel && wpr_buffer[0] > OversoldLevel)
     {
      if(!is_trade_open)
        {
         ...
         trade.Buy(LotSize, _Symbol, price, sl, tp, "WPR Buy Signal");
        }
     }

//--- check for sell signal (WPR returns from overbought)
   if(wpr_buffer[1] >= OverboughtLevel && wpr_buffer[0] < OverboughtLevel)
     {
      if(!is_trade_open)
        {
         ...
         if(StopLossPips == 0) sl = 0;
         if(TakeProfitPips == 0) tp = 0;
         trade.Sell(LotSize, _Symbol, price, sl, tp, "WPR Sell Signal");
        }
     }
  }
//+------------------------------------------------------------------+
