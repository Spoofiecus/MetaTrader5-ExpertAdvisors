...
input int                MagicNumber = 707070;   // Magic Number
...
//--- get MFI handle
   ...
   if(mfi_handle == INVALID_HANDLE)
     {
      printf("Error creating MFI indicator");
      return(INIT_FAILED);
     }
...
//--- get MFI values for the last 2 completed bars
   if(CopyBuffer(mfi_handle, 0, 1, 2, mfi_buffer) != 2)
     {
      printf("Error copying MFI buffer");
      return;
     }
...
//--- check for buy signal (MFI returns from oversold)
   if(mfi_buffer[1] <= OversoldLevel && mfi_buffer[0] > OversoldLevel)
     {
      if(!is_trade_open)
        {
         ...
         trade.Buy(LotSize, _Symbol, price, sl, tp, "MFI Buy Signal");
        }
     }

//--- check for sell signal (MFI returns from overbought)
   if(mfi_buffer[1] >= OverboughtLevel && mfi_buffer[0] < OverboughtLevel)
     {
      if(!is_trade_open)
        {
         ...
         if(StopLossPips == 0) sl = 0;
         if(TakeProfitPips == 0) tp = 0;
         trade.Sell(LotSize, _Symbol, price, sl, tp, "MFI Sell Signal");
        }
     }
  }
//+------------------------------------------------------------------+
