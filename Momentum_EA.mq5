...
//--- get Momentum handle
   momentum_handle = iMomentum(_Symbol, _Period, MomentumPeriod, AppliedPrice);
...
//--- get Momentum values for the last 2 completed bars
   if(CopyBuffer(momentum_handle, 0, 1, 2, momentum_buffer) != 2)
     {
      printf("Error copying Momentum buffer");
      return;
     }
...
//--- check for buy signal (Momentum crosses above 100)
   if(momentum_buffer[1] <= 100 && momentum_buffer[0] > 100)
     {
      if(!is_trade_open)
        {
         ...
         trade.Buy(LotSize, _Symbol, price, sl, tp, "Momentum Buy Signal");
        }
     }

//--- check for sell signal (Momentum crosses below 100)
   if(momentum_buffer[1] >= 100 && momentum_buffer[0] < 100)
     {
      if(!is_trade_open)
        {
         ...
         if(StopLossPips == 0) sl = 0;
         if(TakeProfitPips == 0) tp = 0;
         trade.Sell(LotSize, _Symbol, price, sl, tp, "Momentum Sell Signal");
        }
     }
  }
//+------------------------------------------------------------------+
